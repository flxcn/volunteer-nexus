...
if(isset($_GET["opportunity_id"]) && !empty(trim($_GET["opportunity_id"]))) {
    ...
    if($obj->getOpportunity($opportunity_id)) {
        ...
        $limit_per_volunteer = $obj->getLimitPerVolunteer();
        ...
        echo "Opportunity details not available";
    }
}
else {
    header("location: error.php");
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    {
        if($obj->updateOpportunity($opportunity_id)) 
        {
        ...
        <div class="row">
            <div class="col-md-12 d-flex justify-content-center order-md-1">
                <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" class="row g-2">
                    ...
                    <button class="w-100 btn btn-primary btn-lg btn-block" type="submit">Update opportunity</button>
