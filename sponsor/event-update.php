...
if(isset($_POST["event_id"]) && !empty($_POST["event_id"])) {
    ...
    if(isset($_GET["event_id"]) && !empty(trim($_GET["event_id"]))){
        ...
        if($obj->getEvent($event_id)) {
            $event_name = $obj->getEventName(); 
            ...
            $registration_start = $obj->getRegistrationStart();
            $registration_end = $obj->getRegistrationEnd();
            $event_start = $obj->getEventStart();
        ...
        <div class="row">
            <div class="col-md-12 d-flex justify-content-center order-md-1">
                <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" class="row g-2">
                    ...
                    <input type="hidden" name="sponsor_name" value="<?php echo $sponsor_name; ?>"/>
                    <input type="hidden" name="contribution_type" value="<?php echo $contribution_type; ?>"/>
                    <input type="hidden" name="event_id" value="<?php echo $event_id; ?>"/>

                    <button class="w-100 btn btn-primary btn-lg btn-block" type="submit">Update event</button>
