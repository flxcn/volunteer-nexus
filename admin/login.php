...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($username_error) && empty($password_error)){
        ...
        $sql = "SELECT admin_id, first_name, last_name, username, password FROM administrators WHERE username = ?";

        if($query = mysqli_prepare($link, $sql)){
            mysqli_stmt_bind_param($query, "s", $param_username);
            ...
            if(mysqli_stmt_execute($query)){
                ...
                if(mysqli_stmt_num_rows($query) == 1){
                    mysqli_stmt_bind_result($query, $admin_id, $first_name, $last_name, $username, $hashed_password);
